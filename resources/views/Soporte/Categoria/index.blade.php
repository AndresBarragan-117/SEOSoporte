...
<div>
	...
	<form action="{{ url('categoria')}}" role="form" class="form-horizontal" method="POST" accept-charset="utf-8">
		{{ csrf_field() }}
		
		<div style="margin-left:6px;margin-bottom: 12px;">
			<a href="{{ url('categoria') }}" class="btn btn-info"><span class="fa fa-file" title="Limpiar"></span></a>
			<a href="{{ url('categoria/show') }}" class="btn btn-info" title="Consultar"><span class=" fa fa-search" ></span></a>
			...
			<div role="tabpanel" class="tab-pane active" id="form">
				<div class="row">
					<div class="col-md-8">
						...
						<input name="codigo" id="codigo" class="form-control" value="{{old('codigo')}}"></input>
						<div class="text-danger">{!!$errors->first('codigo', '<small>:message</small>')!!}</div>
					</div>
				</div>
			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					...
					<tr>
						<td>{{$dt->nombre}}</td>
						<td>
							<form id="formDelete" action="{{ route('categoria.destroy', $dt->idCategoria) }}" method="POST">
								<input type="hidden" value="DELETE" name="_method">
								{{ csrf_field() }}
								<a href="{{ $dt->idCategoria.'/edit'}}" class="btn btn-success btn-xs"><span class="fa fa-check"></span></a>
