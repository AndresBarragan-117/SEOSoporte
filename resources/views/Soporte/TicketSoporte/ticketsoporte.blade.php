...
<div>
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ url('ticketSoporte')}}" role="form" accept-charset="utf-8">
        @csrf

        <div style="margin-left:6px;margin-bottom: 12px;">
			<a href="{{ url('ticketSoporte') }}" class="btn btn-info"><span class="fa fa-file" title="Limpiar"></span></a>
        ...
        <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="form">
                ...
                <div class="row">
                    <div class="col-md-10">
                        <label for="tareaRapida"><b>Tarea Rápida?</b></label>
                        @if(old('tareaRapida', '') == 'on')
                            <input type="checkbox" id="tareaRapida" name="tareaRapida" checked />
                        @else
                            <input type="checkbox" id="tareaRapida" name="tareaRapida" />
                        @endif
                    </div>
                </div>
                <div class="row" id="rowSolucion">
                    <div class="col-md-10">
                        <label for="solucion"><b>Solución</b></label>
                        <textarea rows="10" name="solucion" maxlength="500" id="solucion" class="form-control ckeditor">{{old('solucion')}}</textarea>
                        <div class="text-danger">{!!$errors->first('solucion', '<small>:message</small>')!!}</div>
                    </div>
                </div>
            </div>
