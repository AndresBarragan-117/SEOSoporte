...
<div>
	...
	<form action="{{ url('ticketEstado')}}" role="form" class="form-horizontal" method="POST" accept-charset="utf-8">
		{{ csrf_field() }}
		...
		<div style="margin-left:6px;margin-bottom: 12px;">
			...
			<a href="{{ url('ticketEstado/show') }}" class="btn btn-info" title="Consultar"><span class=" fa fa-search" ></span></a>
			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					...
					<tr>
						<td>{{$dt->nombre}}</td>
						<td>{{$dt->orden}}</td>
						<td style="text-align: center;"><input disabled="true" type="color" value="{{$dt->color}}"></td>
						<td>
							<form id="formDelete" action="{{ route('ticketEstado.destroy', $dt->idTicketEstado) }}" method="POST">
								<input type="hidden" value="DELETE" name="_method">
								{{ csrf_field() }}
								<a href="{{ $dt->idTicketEstado.'/edit'}}" class="btn btn-success btn-xs"><span class="fa fa-check"></span></a>
